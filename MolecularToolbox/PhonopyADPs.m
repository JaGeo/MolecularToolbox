classdef PhonopyADPs
    ...
    properties(GetAccess = 'public', SetAccess = 'private')
        ...
        %Should be transformed to a better format in the future
        %getaccess for each atom and each Uij should be a sufficient
        %solution
        UcartReader
        ...
        %File format of thermal_displacement_matrices.yaml > 1.11.2
        Phonopy1_11_2
    end
    ...
    methods
        ...
        function obj=PhonopyADPs(filenamethermals,TSTART,TEND,TSTEP,NumberOfAllAtoms)
            ...
            %File format of version 1.11.2?
            try
                fid2=fopen(filenamethermals);
               %ignore lines
                for i = 1:7+obj.NumberOfAllAtoms
                        
                        if i==7+obj.NumberOfAllAtoms
                            text=fgets(fid2);
                        else
                            fgets(fid2);
                        end
            ...
            found = strfind(text,'displacement_matrices_cif:');
                if (isempty(found)==false)
                    obj.Phonopy1_11_2=true;
                end
            
            fclose(fid2);
            
            %File format of version 1.11.2
             if (obj.Phonopy1_11_2==true)
                %opens file
                try
                    fid2=fopen(filenamethermals);
                ...
                try
                    ...
                    for TemperatureCounter=1:obj.NumberOfTemperatures
                        ...
                        for i = 1:4+obj.NumberOfAllAtoms
                            
                            if i==3+obj.NumberOfAllAtoms
                                text=fgets(fid2);
                            else
                                fgets(fid2);
                            end
